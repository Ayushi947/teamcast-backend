...
export const seedJobAiAssessments = async (jobApplications: any[]) => {
  try {
    // logger.info("Starting job assessment seed...");

    // Clear existing job assessments
    await prisma.job_ai_assessment.deleteMany();

    // Create job assessments for each candidate
    const assessments = await Promise.all(
      jobApplications.map(async (jobApplication) => {
        ...
        const assessment = await prisma.job_ai_assessment.create({
          data: {
        ...
        if (isCompleted) {
          ...
          for (let i = 0; i < sections.length; i++) {
            await prisma.job_ai_assessment_section.create({
              data: {
    ...
    logger.info(`Created ${assessments.length} job assessments successfully`);
    return assessments;
  } catch (error) {
    logger.error('Error seeding job assessments:', error);
    throw error;
  }
};
