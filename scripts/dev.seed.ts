...
import { createPageSpecificOnboardingTours } from './dev.seed/page-specific-candidate-onboarding-tour';
...
async function main() {
  try {
    ...
    logger.error('Error seeding development data:', e);
    throw e;
  } finally {
...
async function createSupportUser(name: string, email: string) {
  try {
    ...
    logger.error('Failed to create support user', {
      ...
      name,
      email,
      context: 'DevelopmentSeed.createSupportUser',
    });
    throw error;
  }
}
...
async function createSubscriptionPackages() {
  ...
  // Create client subscription packages with payment provider information
  await Promise.all([
    // STARTER Tier
...
const seed = async () => {
  try {
    await createPageSpecificOnboardingTours();
    logger.info('Tours seed completed successfully');
  } catch (error) {
