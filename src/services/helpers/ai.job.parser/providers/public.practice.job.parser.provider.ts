...
// Schema definitions for validation - more lenient for public practice assessments
const JobRequirementSchema = z.object({
  skill: z.string(),
  required: z.boolean(),
  experience: z.number().nullable().optional(), // Allow null for experience
});
...
/**
 * Job parser provider specifically for public practice assessments
 * This provider handles web URLs directly without requiring file uploads to storage
 */
@singleton
export class PublicPracticeJobParserProvider implements IJobParserProvider {
  ...
  constructor() {
    ...
    if (this.useRedisCache) {
      try {
        this.redis = new Redis({
          ...
          keyPrefix: `${ENV.ENV_NAME}:public_practice_job_parser:`,
          connectTimeout: 5000,
        ...
        this.redis.on('error', (err: Error) => {
          logger.error('Redis error', {
            error: err.message,
            context: 'PublicPracticeJobParserProvider',
          });
        });

        logger.info('Redis cache initialized for public practice job parser', {
          context: 'PublicPracticeJobParserProvider.constructor',
        });
      } catch (error) {
      ...
      logger.info(
        'Using local Map storage for public practice job parser tasks',
        {
          context: 'PublicPracticeJobParserProvider.constructor',
        }
      );
    ...
    logger.info('Public Practice Job Parser Provider initialized', {
      context: 'PublicPracticeJobParserProvider.constructor',
      model: this.model,
    ...
    if (this.useRedisCache && this.redis) {
      try {
        ...
        logger.error('Failed to set task in Redis', {
          error: error instanceof Error ? error.message : 'Unknown error',
          taskId,
          context: 'PublicPracticeJobParserProvider.setTask',
        });
        // Fallback to Map if Redis fails
    ...
    if (this.useRedisCache && this.redis) {
      try {
        ...
        logger.error('Failed to delete task from Redis', {
          error: error instanceof Error ? error.message : 'Unknown error',
          taskId,
          context: 'PublicPracticeJobParserProvider.deleteTask',
        });
        // Fallback to Map if Redis fails
  ...
  private detectMimeType(fileUrl: string, fileBuffer: Buffer): string {
    ...
    // For web URLs, check content type from response headers if available
    // For now, assume HTML/text content for web URLs
    if (url.startsWith('http://') || url.startsWith('https://')) {
      // Check if it's likely HTML
      const textSample = fileBuffer.subarray(
        0,
        Math.min(100, fileBuffer.length)
      );
      const text = textSample.toString('utf-8').toLowerCase();
  ...
  /**
   * Downloads the job description from a web URL
   * @param fileUrl The URL of the job description (web URL)
   * @returns The file content as a Buffer
   */
  private async downloadJobFile(fileUrl: string): Promise<Buffer> {
    try {
      logger.info('Downloading job description from web URL', {
        context: 'PublicPracticeJobParserProvider.downloadJobFile',
        fileUrl,
      });
      ...
      const response = await axios.get(fileUrl, {
        ...
        headers: {
          Accept:
            'text/html,application/xhtml+xml,application/xml,text/plain,application/pdf,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,*/*',
          'User-Agent':
      ...
      // Check if it's a 403 Forbidden error (common for sites like Indeed that block scraping)
      const is403Error =
      ...
      if (is403Error) {
        throw new AppError(
          'Unable to access this job URL. The website may be blocking automated access. Please copy and paste the job description text instead.',
          403,
          ErrorCode.FORBIDDEN
        );
      }
  ...
  private async attemptJobParsingWithModel(
    ...
    logger.debug('Prompt for job parsing', {
      context: 'PublicPracticeJobParserProvider.attemptJobParsingWithModel',
      mode,
      prompt,
      attempt,
    });
    ...
    // Handle text/HTML files differently
    if (mimeType === 'text/plain' || mimeType === 'text/html') {
      // For text/HTML files, include the content directly as text
      const textContent = jobFile.toString('utf-8');
      // If HTML, try to extract text content (basic extraction)
      let cleanContent = textContent;
      if (mimeType === 'text/html') {
        // Basic HTML tag removal (keep text content)
   ...
   * @param taskId The task ID
   * @param jobUrl The URL of the job description to parse (must be a web URL)
  ...
  async parse(
    ...
    // Validate that the URL is a web URL or data URL
    if (
      !jobUrl.startsWith('http://') &&
      !jobUrl.startsWith('https://') &&
      !jobUrl.startsWith('data:')
    ) {
      throw new AppError(
        'Public practice job parser only supports web URLs (http/https) or data URLs',
        400,
  ...
  private async parseJobDescription(
    ...
    try {
      ...
      // Download the job description file from web URL or extract from data URL
      task.status = IAiJobParsingStatus.DOWNLOADING_JOB_DESCRIPTION;
      await this.setTask(taskId, task);

      let jobFile: Buffer;
      if (task.fileUrl.startsWith('data:')) {
        // Handle data URL - extract base64 content
        const base64Match = task.fileUrl.match(/^data:[^;]+;base64,(.+)$/);
        if (!base64Match) {
          throw new AppError(
        ...
        jobFile = Buffer.from(base64Match[1], 'base64');
      } else {
        // For public practice assessments, download directly from web URL
        jobFile = await this.downloadJobFile(task.fileUrl);
      }
      ...
      for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
          ...
          logger.info('Job parsing successful', {
            context: 'PublicPracticeJobParserProvider.parseJobDescription',
            attempt,
  ...
  private parseVertexAIResponse(
    ...
    try {
      ...
      logger.error('Failed to parse validated data', {
        context: 'PublicPracticeJobParserProvider.parseVertexAIResponse',
        error: error instanceof Error ? error.message : 'Unknown error',
        data: validatedData,
      });
  ...
  private validateString(
    ...
    if (!value || value === null) {
      logger.debug(`Invalid ${fieldName}, using default empty string`, {
        context: 'PublicPracticeJobParserProvider.validateString',
        fieldName,
        value,
      });
      return '';
  ...
  private validateNumber(
    ...
    if (value === undefined || value === null || isNaN(value)) {
      logger.debug(`Invalid ${fieldName}, using default 0`, {
        context: 'PublicPracticeJobParserProvider.validateNumber',
        fieldName,
        value,
      });
      return 0;
  ...
  private validateBoolean(
    ...
    if (value === undefined || value === null) {
      logger.debug(`Invalid ${fieldName}, using default false`, {
        context: 'PublicPracticeJobParserProvider.validateBoolean',
        fieldName,
        value,
      });
      return false;
  ...
  private validateArray<T>(
    ...
    if (!Array.isArray(value) || value === null) {
      logger.debug(`Invalid ${fieldName}, using default empty array`, {
        context: 'PublicPracticeJobParserProvider.validateArray',
        fieldName,
        value,
      });
      return [];
  ...
  private validateDate(
    ...
    if (!value || value === null) {
      logger.debug(`Invalid ${fieldName}, using undefined`, {
        context: 'PublicPracticeJobParserProvider.validateDate',
        fieldName,
        value,
      });
      return undefined;
  ...
  private validateUrl(
    ...
    try {
      ...
      logger.debug(`Invalid URL for ${fieldName}, using undefined`, {
        context: 'PublicPracticeJobParserProvider.validateUrl',
        fieldName,
        value,
      });
      return undefined;
  ...
  private validateEnum<T extends string>(
    ...
    if (!value || value === null) {
      logger.debug(`Missing ${fieldName}, using default ${defaultValue}`, {
        context: 'PublicPracticeJobParserProvider.validateEnum',
        fieldName,
        value,
      });
      return defaultValue;
